...
/* Return a random index value in the range [lb, ub] based on probablility distribution dist */
...
int generate_inc_distr_vec(double **dist_vec, double *prob, int n, char *name)
{
    ...
    *dist_vec = (double *)malloc(n * sizeof(double));
    ...
    if ((*dist_vec)[n-1] > 1.0+eps || (*dist_vec)[n-1] < 1.0-eps) {
        fprintf(stderr, "propbablilities for %s doesn't sum to 1 within %f\n", name, eps);
        ret = 1;
    }

    /* Force upper value to 1, needed to make sure no random value ends up outside of the distribution */
    (*dist_vec)[n-1] = 1.0;
