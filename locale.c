...
void add_locale(double lat, double lon, double pop_dens)
{
    if (num_locale + 1 > max_locale) {
        max_locale += 10;
        locale_list = (struct locale *)realloc(locale_list, max_locale * sizeof(struct locale));
    }
    ...
    locale_list[num_locale].rbeta = atan((1-FLATTENING)*tan(locale_list[num_locale].rlat));
